...
def _load_sage_attention(device):
    if device.type != "cuda":
        return None
    major, minor = torch.cuda.get_device_capability(device)
    architecture = "120" if major > 9 else f"{major}{minor}"
    for module in (f"sageattention_sm{architecture}", "sageattention"):
        try:
            return import_module(module).sageattn
        except ImportError:
            continue
    return None
...
def attention(
    ...
    sageattn = _load_sage_attention(q.device) if attention_type == "SAGE_ATTENTION" else None
