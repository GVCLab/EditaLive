...
class Direction(nn.Module):
    def __init__(self, motion_dim):
        ...
        # At inference the weight is frozen, so Q = qr(weight) does not depend on
        # the input. Streaming calls this once per frame, so cache the first result
        # and skip the repeated QR (which round-trips bf16 <-> fp32). Training keeps
        # recomputing it, since the weight is still being updated.
        self._Q_cache = None
    ...
    def _get_Q(self):
        ...
        if torch.is_grad_enabled():
            ...
            return Q
        key = (weight.device, weight.dtype)
        if self._Q_cache is None or self._Q_cache_key != key:
            Q, _ = custom_qr(weight)
            self._Q_cache = Q
            self._Q_cache_key = key
