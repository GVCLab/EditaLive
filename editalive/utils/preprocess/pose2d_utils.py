...
def read_img(image, convert='RGB', check_exist=False):
    if isinstance(image, str):
        ...
        with Image.open(image) as img:
            return np.array(img.convert(convert) if convert else img)
    if isinstance(image, np.ndarray):
        return image[..., ::-1] if convert else image
    if isinstance(image, Image.Image):
        return np.asarray(image.convert(convert) if convert else image)
    raise TypeError("Expected an image path, NumPy array, or PIL image.")
...
def load_pose_metas_from_kp2ds_seq(kp2ds_seq, width, height):
    ...
    for kps in kp2ds_seq:
        ...
        # 排除全部小于0的情况 (on the first frame there is nothing to fall back to)
        if kp2ds_body[:, :2].min(axis=1).max() < 0 and last_kp2ds_body is not None:
